<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/helpers/aprequest.Master" CodeBehind="default.aspx.vb" Inherits="aprequest._default" %>
...
function fetchVendor(text, li) {
	...
	$('vendorid').value = str;
	var txt = new String(li.innerHTML);
	if ((dpos = txt.indexOf("&nbsp;--&nbsp;"))>2) {
	    txt = txt.replace("&nbsp;--&nbsp;", "\n");
	    var nlc = ( txt.match("\n") || []).length + 2;
	    nlc *= 20;
	    $('txtVendor').style.height = nlc + "px";
	    $('txtVendor').value = txt;
	} else {
	    $('txtVendor').value = txt;
	    $('txtVendor').style.height = "20px";
    }
...
function checkSigKeyUp(vntZ) {
    ...
    var xxx = 'val=' + vntZ;  //$('<%=sigPhrase1.ClientID%>').value;
    var ajx = new Ajax.Request(uurl, {
        postBody: xxx,
        onFailure: function(rct) {
                Dialog.alert("ERROR:\n\n" + rct.responseText, { width: 600, height: 300, okLabel: "close" });        
            },
        onSuccess: function(rct) {
            var retData = rct.responseText.evalJSON();
            if ((retData.error > 0) && (retData.error < 16)) {
                //alert(retData.message);
                Dialog.alert(retData.message, { width: 600, height: 200, okLabel: "close" });
            } else {
                $('<%=sigPhrase1.ClientID%>').hide();
                $('<%=lblPhrase1.ClientID%>').hide();
                $('<%=lblInstru.ClientID%>').hide();
                $('<%= btnSubmit.ClientID%>').show();
                $('<%=btnESig.ClientID%>').show();
            }
        }
    });
}

function hndSigLostFocus(e) {
    var element = Event.element(e);
    var strG = String($('<%=sigPhrase1.ClientID%>').value);
    if (strG.length > 0) {
        checkSigKeyUp($('<%=sigPhrase1.ClientID%>').value);
    }
}
...
document.observe('dom:loaded', function() {
    ...
    //$('txtInvDate').observe('click', function() { console.log("Click - txtInvDate");  Event.fire($('btnCalendar'), 'click'); });
    Calendar.setup({ dateField: 'txtDueDate', triggerElement: 'btnCalDue' });
    //$('txtDueDate').observe('click', function() { $('btnCalDue').fire('click'); });
    ...
    $('<%= btnSubmit.ClientID%>').observe('click', hndSaveAll);

    /*      fileUpload(this.form,'recvfileupload.aspx','upload'); 
    *      return false;
    */
    if ($('<%=sigPhrase1.ClientID%>') != undefined) {
        $('<%=sigPhrase1.ClientID%>').observe('keydown', hndSigKeyUp);
        $('<%=sigPhrase1.ClientID%>').observe('blur', hndSigLostFocus);
        $('<%=lblInstru.ClientID%>').show();
        $('<%=btnESig.ClientID%>').hide();
    ...
    if ($('<%=errorLabel.ClientID%>').value != '') Dialog.alert($('<%=errorLabel.ClientID%>').value,
                            { width: 300, height: 100, okLabel: "close" });
    // alert($('<%=errorLabel.ClientID%>').value);
...
<asp:Literal ID="BigSiteName" runat="server" Text=""></asp:Literal>
    <tr><td align="right">Vendor ID:</td>
        <td colspan="2"><textarea name="txtVendor" id="txtVendor" style="width: 100%; height: 20px;" ></textarea></td>
    ...
    <div id="weredone" style="display: none;">
        ...
        <div>Final transaction number is: <div id="realtxn" style="display: inline-block; font-weight: bold;">xxx</div></div>
        <br />
...
<asp:Content ID="Content3" ContentPlaceHolderID="Bottom" runat="server">
    <div id="detailbuttons" style="padding: 0px; margin: 0px auto; width: 715px; float: none; clear: both;" > 
        ...
        <div id="imgbuttons" style="text-align: right; width: 160px; float: right; margin: 0px;" >
            <asp:ImageButton ImageUrl="~/images/paperclip.gif" ID="btnAttach" ClientIDMode="Static" runat="server" TabIndex="-1" />
            <div id="divUL" name="divUL" style="display: none;" >
            </div>
        </div>
    </div>
    <div id="gvw" style="float: none; clear: both; margin: 0px auto; min-height: 200px; width: 730px;">	
...
<asp:Literal ID="litBody" runat="server" Text=""></asp:Literal>
    ...
    <div id="footerbuttons" style="width: 690px; margin: 3px auto; float: none; clear: both;" > 
        <div id="footleft" 
            style="width: 514px; padding-left: 10px; float: left; text-align: center; overflow:hidden; border-top: 1px dotted #fcc;">
            <asp:Label ID="lblPhrase1" runat="server" Text="Signing Password/Phrase:"></asp:Label><asp:TextBox id="sigPhrase1" TextMode="Password" cssclass="align-center" runat="server"></asp:TextBox><br />
            <asp:Label ID="lblInstru" Visible="false" runat="server" Text="type your signing phrase here to enable the 'save' button"></asp:Label>
            <asp:ImageButton ImageUrl="" ID="btnESig" TabIndex="-1" Height="100px" ClientIDMode="Static" Visible="false" runat="server" />
        </div>
        <div id="footright" 
            style="width: 142px; padding-right: 10px; float: left; text-align: right;">
            <asp:Button ID="btnSubmit" runat="server" Text="Submit Request" Height="25px" CssClass="button-primary" ClientIDMode="Static" />
        </div>
    </div>
</asp:Content>